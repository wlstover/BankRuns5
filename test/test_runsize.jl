...
# Stub the two structs runSize touches, then eval the REAL runSize source.
...
i = findfirst("function runSize(mod::Model)", src)[1]
j = findnext("\nend", src, i)[1] + 3
eval(Meta.parse(src[i:j]))
println("evaluated real runSize definition\n")
...
println("\n✅ runSize: counts and deposit-weights the withdrawal set; empty case guarded")
