...
This experiment is the central test of the chapter's theoretical contribution. Bank-failure probability is plotted as a function of μ ∈ {0.0, 0.25, 0.5, 0.75, 1.0}, both pooled and stratified by the (λ_I, λ_C) gap. Three competing hypotheses are evaluated:

1. **Monotonically increasing in μ** (Prediction 1, first-order effect): individualists are more "trigger-happy" because their Geometric prior systematically over-estimates withdrawal counts after small exogenous shocks. If the first-order effect dominates, run probability should rise smoothly with μ.
2. **Non-monotonic with an interior peak** (Prediction 2, second-order interaction): individualist-driven noise withdrawals are amplified by collectivist neighbors through the social-signal channel, producing cascades that neither homogeneous population would have generated. If the second-order interaction dominates, run probability should peak at intermediate μ.
3. **Approximately flat or noisy in μ**: if the cultural-signal mechanism is weak relative to the baseline parameters (reserve ratio, deposit insurance, σ), μ should have negligible marginal effect.

The shape of the μ curve at different (λ_I, λ_C) gaps directly tests Prediction 3: a larger gap should produce a more pronounced non-monotonic peak. Quantitative results pending sweep completion.
